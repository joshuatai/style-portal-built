!function($) {
    ...
    var Token = function(element, options) {
      ...
      this.elementWidth     = this.options.fieldWidth === 'auto' ? this.$element.outerWidth() : this.options.fieldWidth;
    ...
    Token.prototype = $.extend({}, _super.Constructor.prototype, {
      ...
      bindEvents: function () {
        ...
        this.$input
          ...
          .on('focus', $.proxy(this.focusEvent, this));
        this.$filterCloseBtn
          .on('mouseup', $.proxy(this.removeAllTags, this)); // use mousedown to prevent the autocomplete menu show first when click the close button.
      },
      ...
      focus: function() {
        var _self = this;
        _super.Constructor.prototype.focus.call(this);
        _self.$input.get(0).focus();
        _self.$input.autocomplete('search');
      },
      setMenuWidth: function(){
        var _self = this;
        _self.$input.data('ui-autocomplete').menu.element.css({'width': _self.$wrapper.outerWidth(), 'min-width': _self.$wrapper.outerWidth()});
      }
    });
